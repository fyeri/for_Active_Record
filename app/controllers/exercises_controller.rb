class ExercisesController < ApplicationController
  def exercise1
    ...
    @foods = Food.left_outer_joins(:order_foods).where(order_foods: { id: nil }).distinct
  end
  ...
  def exercise3 
    ...
    @address = Address.joins(:orders).select('addresses.*, count(orders.*) as orders_count').group('addresses.id').first
  end
